...
pub struct PipelineManager {
    pipelines: HashMap<Id, (PipelineDescriptor, RenderPipeline)>,
}

impl PipelineManager {
    pub fn get_pipeline(&self, id: Id) -> Option<&(PipelineDescriptor, RenderPipeline)> {
    ...
    pub fn sorted_pipelines(&self) -> Vec<(Id, &PipelineDescriptor, &RenderPipeline)> {
        let mut sorted_pipelines: Vec<(Id, &PipelineDescriptor, &RenderPipeline)> = self
            .pipelines
            .iter()
            .map(|(id, (descriptor, pipeline))| (*id, descriptor, pipeline))
            .collect();
